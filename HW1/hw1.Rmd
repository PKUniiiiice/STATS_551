...
First, we use a Bayesian Network to represent their relationships. We
use the following information to construct this DAG.
...
\begin{aligned}
&f(x,y,u,v,w)=f_Y(y)f_{X|Y}(x)f_{Z|X,Y}(z)f_{U|X,Y}(u)f_{V|X,Y}(v)f_{W|X,Y}(w)
\end{aligned}
$$
...
So $\mathbf{V}|X,Y$ is a vector random variable with the distribution $$
\begin{aligned}
    &\mathbf{V}|X\land Y = (V_1|X\land Y, V_2|X\land Y, \ldots, V_{24}|X\land Y) \\
    &\text{where } V_i|X\land Y \sim Beta(\alpha_{XY}, \beta_{XY}), \quad i.i.d.\\
\end{aligned}
    $$ The pdf of the joint distribution of $\mathbf{V}|X\land Y$is 
    $$f(\mathbf{V}|X \land Y) = \prod_{i=1}^{24} \frac{v_i^{\alpha_{XY} - 1} (1 - v_i)^{\beta_{XY} - 1}}{B(\alpha_{XY}, \beta_{XY})}
    $$
...
$$ we have $$
\begin{aligned}
P(\theta=0.6|T_1, T_2)&=\frac{(1-0.6)*(1-0.6)*1/2}{0.6*0.6*1/2+0.4*0.4*1/2}=\frac{4}{13}, 
...
\begin{aligned}
\mathbb{E}(Y|T_1, T_2)
&=\mathbb{E}(\mathbb{E}(Y|\theta)|T_1, T_2)\\
&=\Sigma_{\theta} P(\theta|T_1, T_2)\cdot \mathbb{E}\left( \left.Y \right|T_1, T_2,\theta \right)\\
